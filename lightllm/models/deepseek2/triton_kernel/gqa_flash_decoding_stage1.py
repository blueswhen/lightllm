...
@triton.jit
def _fwd_kernel_flash_decode_stage1_fp8(
    Q_nope,
    Q_nope_scale,
    Q_rope,
    Q_rope_scale,
    ...
    off_q = cur_batch * stride_q_bs + cur_q_head_range[:, None] * stride_q_h + offs_d[None, :]
    off_q_scale = cur_batch * Q_HEAD_NUM + cur_q_head_range[:, None]
    off_rope_q = cur_batch * stride_q_rope_bs + cur_q_head_range[:, None] * stride_q_rope_h + offs_rope_d[None, :]
    off_rope_q_scale = cur_batch * Q_HEAD_NUM + cur_q_head_range[:, None]
    q = tl.load(Q_nope + off_q)
    q_scale = tl.load(Q_nope_scale + off_q_scale)
    q_rope = tl.load(Q_rope + off_rope_q)
    q_rope_scale = tl.load(Q_rope_scale + off_rope_q_scale)
    block_n_size = (
        tl.where(
            cur_batch_end_index - cur_batch_start_index <= 0,
            0,
    ...
    max_logic = tl.zeros([Q_HEAD_NUM], dtype=tl.float32) - float("inf")
    acc = tl.zeros([Q_HEAD_NUM, BLOCK_DMODEL], dtype=tl.float32)
    # att_value = tl.zeros([Q_HEAD_NUM, 32], dtype=tl.float32)
    for start_n in range(0, block_n_size, 1):
        offs_n_new = start_n * BLOCK_N + offs_n
        kv_loc = tl.load(
            Req_to_tokens + stride_req_to_tokens_b * cur_batch_req_idx + offs_n_new,
        ...
        off_kv = kv_loc[None, :] * stride_kv_bs + offs_d[:, None]
        off_kv_scale = kv_loc[None, :]
        kv = tl.load(KV_nope + off_kv, mask=offs_n_new[None, :] < cur_batch_end_index, other=0.0)
        kv_scale = tl.load(KV_nope_scale + off_kv_scale, mask=offs_n_new[None, :] < cur_batch_end_index, other=0.0)
        att_value = tl.dot(q, kv) * q_scale * kv_scale
        off_rope_kv = kv_loc[None, :] * stride_kv_rope_bs + offs_rope_d[:, None]
        off_rope_kv_scale = kv_loc[None, :]
        rope_kv = tl.load(KV_rope + off_rope_kv, mask=offs_n_new[None, :] < cur_batch_end_index, other=0.0)
        rope_kv_scale = tl.load(
            KV_rope_scale + off_rope_kv_scale, mask=offs_n_new[None, :] < cur_batch_end_index, other=0.0
        )
        att_value += tl.dot(q_rope, rope_kv) * q_rope_scale * rope_kv_scale
        ...
        logic_scale = tl.exp(max_logic - new_max_logic)
        acc *= logic_scale[:, None]
        acc += tl.dot(exp_logic.to(tl.bfloat16), (kv * kv_scale).to(tl.bfloat16).T)
...
def flash_decode_stage1(
    ...
    batch, q_head_num = B_req_idx.shape[0], q_nope.shape[1] if not use_fp8_w8a8 else q_nope[0].shape[1]
    if q_head_num % BLOCK_Q_HEAD == 0:
        grid = (triton.cdiv(max_len_in_batch, BLOCK_SEQ), q_head_num // BLOCK_Q_HEAD, batch)
        if use_fp8_w8a8:
            _fwd_kernel_flash_decode_stage1_fp8[grid](
                q_nope[0],
                q_nope[1],
                q_rope[0],
                q_rope[1],
                ...
                BLOCK_SEQ=BLOCK_SEQ,
                BLOCK_DMODEL=q_nope[0].shape[-1],
                BLOCK_ROPE_DMODEL=q_rope[0].shape[-1],
                BLOCK_N=BLOCK_N,
                num_warps=num_warps,
